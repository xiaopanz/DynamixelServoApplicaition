...
#include "ui_secondlayoutcontinuousreverseskinslipwindow.h"

SecondLayoutContinuousReverseSkinSlipWindow::SecondLayoutContinuousReverseSkinSlipWindow(ServoUtility *servoUtilityPtr, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SecondLayoutContinuousReverseSkinSlipWindow),
...
void SecondLayoutContinuousReverseSkinSlipWindow::on_angleSlider_valueChanged(int value)
{
    ui->angleVal->setText(QString::number(value));
}


void SecondLayoutContinuousReverseSkinSlipWindow::on_velocitySlider_valueChanged(int value)
{
    ui->velocityVal->setText(QString::number(value));
}


void SecondLayoutContinuousReverseSkinSlipWindow::on_resetButton_clicked()
{
...
void SecondLayoutContinuousReverseSkinSlipWindow::on_startButton_clicked()
{
    ui->startButton->setEnabled(false);
    int direction = 0;
    ...
    qDebug() << "Finished performing continuous reverse skin slip!";
}


void SecondLayoutContinuousReverseSkinSlipWindow::on_quitButton_clicked()
{
    ui->startButton->setEnabled(false);
    // Change servos velocity
    for (int i = 0; i < NUM_OF_DXL_2; i++) {
